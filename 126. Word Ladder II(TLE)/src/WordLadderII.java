...
/**
 * before while loop:  wordSet,    res,   beginSet,  prevseen,  tmpseen
 * before for loop:             tmpres, tmpbeginSet
 * inside for loop:                     subbeginSet
 *
 * NOTE: it is hard and slow to find a bug using Java visualizer or debugger result for complicated problems.
 * instead, use System.out.println to certify the result in every steps will be much easier and faster.
 *
 * Note: this way is TLE , 29 / 39 test cases passed. .
 */
class Solution {
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        if(wordList.contains(beginWord)) prevseen.add(beginWord);
        List<String> list = new LinkedList<>();
        list.add(beginWord);
        res.add(list);

        while (!beginSet.isEmpty() && prevseen.size() != M && !found){
            List<List<String>> tmpres = new LinkedList<>();
            Set<String> tmpbeginSet = new HashSet<>();
            tmpseen.clear();
            int idx = 0, ii;
            for (ii = 0; ii < res.size(); ii++) {
                List<String> l = res.get(ii);
                Set<String> subbeginSet = new HashSet<>();
                String w = l.get(l.size()-1);
                ...
                for (int i = 0; i < n; i++) {
                    for (char c = 'a'; c < 'z'; c++) {
                        ...
                        if (wordList.contains(target) && !prevseen.contains(target)) {
                            subbeginSet.add(target);
                ...
                if(subbeginSet.isEmpty()  ||  l.size() == M-1 && !subbeginSet.contains(endWord) )
                    continue;
                if(found && !subbeginSet.contains(endWord))
                    continue;
                for(String lastword : subbeginSet){
                    if((l.size() == M-1 || subbeginSet.contains(endWord)) && !lastword.equals(endWord))
                        continue;
                    if(lastword.equals(endWord)) {
                        found = true;
                    }
                    tmpres.add(new LinkedList<>(l));
                    tmpres.get(idx++).add(lastword);
                }
                /*System.out.println("subbeginSet:"+subbeginSet);*/
            }
            prevseen.addAll(tmpseen);
            beginSet = tmpbeginSet;
            res = tmpres;
            /*System.out.println("beginSet:"+beginSet);
            System.out.println("res:"+res);
            System.out.println("prevseen.size():"+prevseen.size()+",M="+M);
            System.out.println("beginSet.size():"+beginSet.size());
            System.out.println("**********************");*/
        }
        ...
        for(List<String> list2: res){
            String lastw = list2.get(size-1);
            if(lastw.equals(endWord))
                tmpres.add(list2);
        }
        res = tmpres;
        return res;
    ...
    public void start() {
        /*String[] wordArray = {"hot","dog","lot","log","cog","dot"};
        List<String> wordList = ArraytoList(wordArray);
        System.out.println(findLadders("hit","cog",wordList));

        String[] wordArray1 = {"a","b","c"};
        List<String> wordList1 = ArraytoList(wordArray1);
        System.out.println(findLadders("a","c",wordList1));

        String[] wordArray2 = {"hot","dot","dog"};
        List<String> wordList2 = ArraytoList(wordArray2);
        System.out.println(findLadders("hot","dot",wordList2));

        String[] wordArray3 = {"peale","wilts","place","fetch","purer","pooch","peace","poach","berra","teach","rheum","peach"};
        List<String> wordList3 = ArraytoList(wordArray3);
        System.out.println(findLadders("teach","place",wordList3));*/

        String[] wordArray4 = {"slit","bunk","wars","ping","viva","wynn","wows",
                "irks","gang","pool","mock","fort","heel","send","ship",
                "cols","alec","foal","nabs","gaze","giza","mays","dogs",
                "karo","cums","jedi","webb","lend","mire","jose","catt",
                "grow","toss","magi","leis","bead","kara","hoof","than",
                "ires","baas","vein","kari","riga","oars","gags","thug",
                "yawn","wive","view","germ","flab","july","tuck","rory",
                "bean","feed","rhee","jeez","gobs","lath","desk","yoko",
                "cute","zeus","thus","dims","link","dirt","mara","disc",
                "limy","lewd","maud","duly","elsa","hart","rays","rues",
                "camp","lack","okra","tome","math","plug","monk","orly",
